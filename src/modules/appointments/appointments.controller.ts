...
import { sendBadRequestError, sendConflictError, sendNotFoundError } from '@/shared/errors';
...
export async function createAppointmentController(
  ...
  try {
    ...
    if (error.message === 'Doctor not available at this time') {
      return sendBadRequestError(reply, 'Médico não disponível neste horário.');
    }
    if (
      error.message === 'Doctor already has an appointment at this time' ||
...
export async function cancelAppointmentController(
  ...
  try {
    ...
    if (error.message === 'Appointment already cancelled') {
      return sendBadRequestError(reply, 'Agendamento já cancelado.');
    }
    if (error.message === 'Cannot cancel appointment less than 2 hours before') {
      return sendBadRequestError(reply, 'Não é possível cancelar com menos de 2 horas de antecedência.');
    }
    throw error;
  }
}
